 ...
 * @param {object|array} opts.range Range quer(ies) by which to constrain the search
...
function buildQuery(opts) {
	...
	if (opts.range) {
		opts.range = !Array.isArray(opts.range) ? [opts.range] : opts.range;
		opts.range.forEach(r => {
			qbArr.push(qb.range(r.name, r.comparison, r.value));
		});
	}
